...
def _broker(db):
    db.execute("INSERT INTO brokers (broker_name) VALUES ('B') ON CONFLICT DO NOTHING")
...
def _txn(db, bid):
    cur = db.execute("""
        ...
        VALUES (%s, 'Cust', '2026-01-01', 10000, 'Pending') RETURNING transaction_id
    """, (bid,))
    return cur.fetchone()["transaction_id"]


def _vendor(db):
    db.execute("INSERT INTO vendors (vendor_name) VALUES ('Vend') ON CONFLICT DO NOTHING")
...
def test_cih_created_on_cash_payment(db):
    ...
    bid = _broker(db)
    tid = _txn(db, bid)
    _pc = db.execute("""INSERT INTO payments (transaction_id, payment_date, amount, method)
                  VALUES (%s, '2026-01-10', 5000, 'Cash') RETURNING payment_id""", (tid,))
    pid = _pc.fetchone()["payment_id"]
    db.execute("""INSERT INTO cash_in_hand_entries
                  (entry_date, details, amount, txn_type, source_type, source_id)
                  VALUES ('2026-01-10', 'Cust', 5000, 'Credit', %s, %s)""",
               (SRC_CUSTOMER_CASH, pid))
    db.commit()

    row = db.execute(
        "SELECT * FROM cash_in_hand_entries WHERE source_type=%s AND source_id=%s",
...
def test_cih_not_created_for_non_cash_payment(db):
    ...
    db.execute("""INSERT INTO payments (transaction_id, payment_date, amount, method)
                  VALUES (%s, '2026-01-10', 5000, 'UPI')""", (tid,))
    # Simulate: no CIH insert because method != Cash
    db.commit()

    count = db.execute(
        "SELECT COUNT(*) FROM cash_in_hand_entries WHERE source_type=%s",
...
def test_cih_updated_on_payment_amount_edit(db):
    ...
    db.execute("UPDATE payments SET amount=7000 WHERE payment_id=%s", (pid,))
    db.execute("UPDATE cash_in_hand_entries SET amount=7000 WHERE source_type=%s AND source_id=%s",
               (SRC_CUSTOMER_CASH, pid))
    db.commit()

    row = db.execute(
        "SELECT amount FROM cash_in_hand_entries WHERE source_type=%s AND source_id=%s",
        (SRC_CUSTOMER_CASH, pid)).fetchone()
...
def test_cih_deleted_on_payment_delete(db):
    ...
    db.execute("DELETE FROM cash_in_hand_entries WHERE source_type=%s AND source_id=%s",
               (SRC_CUSTOMER_CASH, pid))
    db.execute("DELETE FROM payments WHERE payment_id=%s", (pid,))
    db.commit()
...
def test_cih_deleted_on_method_change_cash_to_upi(db):
    ...
    db.execute("UPDATE payments SET method='UPI' WHERE payment_id=%s", (pid,))
    db.execute("DELETE FROM cash_in_hand_entries WHERE source_type=%s AND source_id=%s",
               (SRC_CUSTOMER_CASH, pid))
    db.commit()
...
def test_cih_created_on_method_change_upi_to_cash(db):
    ...
    bid = _broker(db)
    tid = _txn(db, bid)
    _pc2 = db.execute("""INSERT INTO payments (transaction_id, payment_date, amount, method)
                  VALUES (%s, '2026-01-10', 5000, 'UPI') RETURNING payment_id""", (tid,))
    pid = _pc2.fetchone()["payment_id"]
    db.commit()

    # Simulate method change: UPI → Cash
    db.execute("UPDATE payments SET method='Cash' WHERE payment_id=%s", (pid,))
    db.execute("""INSERT INTO cash_in_hand_entries
                  (entry_date, details, amount, txn_type, source_type, source_id)
                  VALUES ('2026-01-10', 'Cust', 5000, 'Credit', %s, %s)""",
               (SRC_CUSTOMER_CASH, pid))
    ...
    row = db.execute(
        "SELECT txn_type FROM cash_in_hand_entries WHERE source_type=%s AND source_id=%s",
        (SRC_CUSTOMER_CASH, pid)).fetchone()
    assert row is not None
    assert row["txn_type"] == "Credit"
...
def test_cih_cleaned_up_on_transaction_delete(db):
    ...
    for pmt_date, amt in [("2026-01-10", 3000), ("2026-01-20", 2000)]:
        _pc3 = db.execute("""INSERT INTO payments (transaction_id, payment_date, amount, method)
                      VALUES (%s, %s, %s, 'Cash') RETURNING payment_id""", (tid, pmt_date, amt))
        pid = _pc3.fetchone()["payment_id"]
        db.execute("""INSERT INTO cash_in_hand_entries
                      (entry_date, details, amount, txn_type, source_type, source_id)
                      VALUES (%s, 'Cust', %s, 'Credit', %s, %s)""",
    ...
    db.execute("""DELETE FROM cash_in_hand_entries
                  WHERE source_type=%s AND source_id IN (
                      SELECT payment_id FROM payments WHERE transaction_id=%s AND method='Cash'
                  )""", (SRC_CUSTOMER_CASH, tid))
    db.execute("DELETE FROM payments WHERE transaction_id=%s", (tid,))
    db.execute("DELETE FROM customer_transactions WHERE transaction_id=%s", (tid,))
    db.commit()

    # Assert no SRC_CUSTOMER_CASH entries remain (seeded opening balance is a different source_type)
    count = db.execute("SELECT COUNT(*) FROM cash_in_hand_entries WHERE source_type=%s",
                       (SRC_CUSTOMER_CASH,)).fetchone()[0]
    assert count == 0
...
def test_vendor_ub_payment_creates_cih_debit(db):
    ...
    vid = _vendor(db)
    _ec = db.execute("""INSERT INTO vendor_entries
                  (vendor_id, entry_date, ledger_type, firm, entry_kind, particulars, amount)
                  VALUES (%s, '2026-01-15', 'UB', NULL, 'Payment', 'UB Payment', 10000)
                  RETURNING entry_id""",
               (vid,))
    eid = _ec.fetchone()["entry_id"]
    db.execute("""INSERT INTO cash_in_hand_entries
                  (entry_date, details, amount, txn_type, source_type, source_id)
                  VALUES ('2026-01-15', 'Vend', 10000, 'Debit', %s, %s)""",
               (SRC_VENDOR_UB, eid))
    db.commit()
...
def test_vendor_delete_cleans_cih(db):
    ...
    db.execute("""DELETE FROM cash_in_hand_entries
                  WHERE source_type=%s AND source_id IN (
                      ...
                      WHERE vendor_id=%s AND ledger_type='UB' AND entry_kind='Payment'
                  )""", (SRC_VENDOR_UB, vid))
    db.execute("DELETE FROM vendor_entries WHERE vendor_id=%s", (vid,))
    db.execute("DELETE FROM vendors WHERE vendor_id=%s", (vid,))
    db.commit()
...
def test_unique_constraint_prevents_duplicate_cih(db):
    ...
    db.execute("""INSERT INTO cash_in_hand_entries
                  (entry_date, details, amount, txn_type, source_type, source_id)
                  VALUES ('2026-01-10', 'First', 5000, 'Credit', %s, 1)""",
               (SRC_CUSTOMER_CASH,))
    db.commit()
    ...
    with pytest.raises(Exception):
        db.execute("""INSERT INTO cash_in_hand_entries
                      (entry_date, details, amount, txn_type, source_type, source_id)
                      VALUES ('2026-01-10', 'Dupe', 5000, 'Credit', %s, 1)""",
                   (SRC_CUSTOMER_CASH,))
        db.commit()
